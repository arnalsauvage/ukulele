...
public class PanneauInterface extends JPanel implements KeyListener, ActionListener {
	...
	public int sauveFichierTexte(File fichier) {
		...
		try {
			...
			// on marque dans le fichier ou plutôt dans le BufferedWriter qui
			// sert comme un tampon(stream)
			output.write(titreGrille.getText() + System.getProperty("line.separator"));
			// on peut utiliser plusieurs fois méthode write
			output.write(texteGrille.getText().replaceAll("\n", System.getProperty("line.separator")));

			output.flush();
			// ensuite flush envoie dans le fichier, ne pas oublier cette
			// Méthode pour le BufferedWriter
